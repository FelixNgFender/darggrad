...
use crate::{
    Exp, Powf, Powi, Relu, Tanh,
    tensor_shape::TensorShape,
    tensor_storage::{TensorItem, TensorStorage},
};

use std::{
    fmt::{Debug, Display},
    ops::{Add, AddAssign, Div, Index, IndexMut, Mul, Neg, RangeInclusive, Sub},
};
#[derive(PartialEq, Eq, Hash, Debug, Clone)]
pub struct TensorImpl<T: TensorItem> {
    pub(crate) shape: TensorShape,
    storage: TensorStorage<T>,
}

impl<T: TensorItem> TensorImpl<T> {
    ...
    pub(crate) fn data(&self) -> Vec<&T> {
        let mut res = vec![];
        let mut current_indices = vec![0; self.shape.ndim()];
    ...
    pub fn map<F, U>(&self, f: F) -> TensorImpl<U>
    where
        F: Fn(&T) -> U,
        U: TensorItem,
    {
    ...
    pub fn reduce<F>(&self, dim: usize, f: F, keepdim: bool) -> TensorImpl<T>
    where
        F: Fn(&T, &T) -> T,
    {
        assert!(
...
impl<T: TensorItem + PrimInt> TensorImpl<T> {
    ...
    pub fn one_hot(indices: &TensorImpl<T>, num_classes: usize) -> Self {
...
impl<T: TensorItem> Index<&[usize]> for TensorImpl<T> {
    type Output = T;
...
impl<T: TensorItem> IndexMut<&[usize]> for TensorImpl<T> {
    fn index_mut(&mut self, indices: &[usize]) -> &mut Self::Output {
        &mut self.storage[self.shape.ravel_index(indices)]
    }
}

impl<T: TensorItem> Display for TensorImpl<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        fn format_recursive<T: TensorItem>(
...
impl<T: TensorItem + Add> Add for TensorImpl<T> {
    type Output = Self;
...
impl<T: TensorItem + AddAssign> AddAssign for TensorImpl<T> {
    fn add_assign(&mut self, rhs: Self) {
        *self = self.clone() + rhs;
    }
}

impl<T: TensorItem + Neg<Output = T>> Neg for TensorImpl<T> {
    type Output = Self;
...
impl<T: TensorItem + Sub<Output = T>> Sub for TensorImpl<T> {
    type Output = Self;
...
impl<T: TensorItem + Mul> Mul for TensorImpl<T> {
    type Output = Self;
...
impl<T: TensorItem + Div<Output = T>> Div for TensorImpl<T> {
    type Output = Self;
...
impl<T: TensorItem + Exp<Output = T>> Exp for TensorImpl<T> {
    type Output = Self;
...
impl<T: TensorItem + Tanh> Tanh for TensorImpl<T> {
    fn tanh(self) -> Self {
        self.map(|&x| x.tanh())
    }
}

impl<T: TensorItem + Relu> Relu for TensorImpl<T> {
    fn relu(self) -> Self {
        self.map(|&x| x.relu())
    }
}

impl<T: TensorItem + Powf<Output = T>> Powf for TensorImpl<T> {
    type Output = Self;
...
impl<T: TensorItem + Powi<Output = T>> Powi for TensorImpl<T> {
    type Output = Self;

    fn powi(self, exp: i32) -> Self::Output {
        self.map(|&x| x.powi(exp))
    }
}

impl<T: TensorItem> TensorImpl<T> {
    pub(crate) fn max(&self, other: T) -> Self {
        self.map(|&x| if x > other { x } else { other })
    }
}
...
mod tests {
    ...
    fn test_tensor_zeros_basic() {
        let tensor = TensorImpl::<f32>::zeros(vec![2, 3]);
    ...
    fn test_tensor_index() {
        let mut tensor_1d = TensorImpl::<f32>::zeros(vec![5]);
        ...
        let mut tensor_2d = TensorImpl::<i32>::zeros(vec![3, 4]);
        ...
        let mut tensor_3d = TensorImpl::<f64>::zeros(vec![2, 3, 4]);
        tensor_3d[&[0, 0, 0]] = 1.1;
        tensor_3d[&[1, 2, 3]] = 2.2;
        tensor_3d[&[0, 1, 2]] = 3.3;
        tensor_3d[&[1, 0, 1]] = 4.4;
        ...
        let mut tensor_mut = TensorImpl::<i32>::zeros(vec![2, 2]);
        {
    ...
    fn test_tensor_storage_and_consistency() {
        ...
        let mut tensor = TensorImpl::<i16>::zeros(vec![3, 4]);
        let test_cases = vec![
            ([0, 0], 100),
            ([0, 3], 103),
            ([1, 1], 111),
        ...
        let mut tensor_3d = TensorImpl::<f32>::zeros(vec![2, 3, 4]);
    ...
    fn test_map() {
        ...
        let mut tensor = TensorImpl::<i32>::zeros(vec![2, 3]);
        tensor[&[0, 0]] = 1;
        tensor[&[0, 1]] = 2;
        tensor[&[0, 2]] = 3;
        tensor[&[1, 0]] = 4;
        ...
        // TODO: Re-enable support for boolean and String tensors
    ...
    fn test_reduce() {
        ...
        let mut tensor_1d = TensorImpl::<i32>::zeros(vec![5]);
        tensor_1d[&[0]] = 1;
        tensor_1d[&[1]] = 2;
        tensor_1d[&[2]] = 3;
        tensor_1d[&[3]] = 4;
        ...
        let mut small_tensor = TensorImpl::<i32>::zeros(vec![2, 3]);
        small_tensor[&[0, 0]] = 5;
        small_tensor[&[0, 1]] = 2;
        small_tensor[&[0, 2]] = 8;
        small_tensor[&[1, 0]] = 1;
        ...
        let mut single_tensor = TensorImpl::<i32>::zeros(vec![1]);
        ...
        let mut narrow_tensor = TensorImpl::<i32>::zeros(vec![1, 5]);
        for j in 0..5 {
        ...
        let mut tensor_for_permute = TensorImpl::<i32>::zeros(vec![2, 3, 4]);
        let mut value = 1;
        for i in 0..2 {
            for j in 0..3 {
                for k in 0..4 {
        ...
        let mut tensor_for_slice = TensorImpl::<i32>::zeros(vec![4, 5]);
        for i in 0..4 {
            for j in 0..5 {
        ...
        let mut tensor_for_skip = TensorImpl::<i32>::zeros(vec![6, 8]);
        for i in 0..6 {
            for j in 0..8 {
        ...
        let mut complex_tensor = TensorImpl::<i32>::zeros(vec![3, 4, 5]);
        let mut val = 1;
        for i in 0..3 {
            for j in 0..4 {
                for k in 0..5 {
    ...
    fn test_broadcast_op() {
        ...
        let mut tensor_a = TensorImpl::<i32>::zeros(vec![2, 3]);
        let mut tensor_b = TensorImpl::<i32>::zeros(vec![2, 3]);
        ...
        let mut tensor_2d = TensorImpl::<i32>::zeros(vec![2, 3]);
        for i in 0..2 {
            for j in 0..3 {
        ...
        let mut tensor_row = TensorImpl::<i32>::zeros(vec![1, 3]);
        tensor_row[&[0, 0]] = 100;
        tensor_row[&[0, 1]] = 200;
        tensor_row[&[0, 2]] = 300;
        ...
        let mut tensor_col = TensorImpl::<i32>::zeros(vec![2, 1]);
        tensor_col[&[0, 0]] = 1000;
        tensor_col[&[1, 0]] = 2000;
        ...
        let mut tensor_3d = TensorImpl::<i32>::zeros(vec![2, 3, 4]);
        let mut tensor_1d = TensorImpl::<i32>::zeros(vec![3]);
        ...
        let mut tensor_2x4 = TensorImpl::<i32>::zeros(vec![2, 4]);
        let mut tensor_3x5 = TensorImpl::<i32>::zeros(vec![3, 5]);
        ...
        let mut base_tensor = TensorImpl::<i32>::zeros(vec![2, 3, 4]);
        let mut counter = 1;
        for i in 0..2 {
            for j in 0..3 {
                for k in 0..4 {
        ...
        let mut broadcast_1d = TensorImpl::<i32>::zeros(vec![2]);
        broadcast_1d[&[0]] = 100;
        broadcast_1d[&[1]] = 200;
        ...
        let mut slice_base = TensorImpl::<i32>::zeros(vec![4, 5]);
        for i in 0..4 {
            for j in 0..5 {
        ...
        let mut slice_broadcast = TensorImpl::<i32>::zeros(vec![1, 3]);
        ...
        let mut skip_base = TensorImpl::<i32>::zeros(vec![6, 8]);
        for i in 0..6 {
            for j in 0..8 {
        ...
        let mut skip_col = TensorImpl::<i32>::zeros(vec![3, 1]);
        skip_col[&[0, 0]] = 10000;
        skip_col[&[1, 0]] = 20000;
        skip_col[&[2, 0]] = 30000;
        ...
        let mut complex_base = TensorImpl::<i32>::zeros(vec![3, 4, 5]);
        let mut val = 1;
        for i in 0..3 {
            for j in 0..4 {
                for k in 0..5 {
        ...
        let mut complex_broadcast_tensor = TensorImpl::<i32>::zeros(vec![3]);
    ...
    fn test_matrix_multiplication() {
        let mut matrix_a = TensorImpl::<i32>::zeros(vec![2, 3]);
        matrix_a[&[0, 0]] = 1;
        matrix_a[&[0, 1]] = 2;
        matrix_a[&[0, 2]] = 3;
        matrix_a[&[1, 0]] = 4;
        ...
        let mut matrix_b = TensorImpl::<i32>::zeros(vec![3, 2]);
        matrix_b[&[0, 0]] = 7;
        matrix_b[&[0, 1]] = 8;
        matrix_b[&[1, 0]] = 9;
        matrix_b[&[1, 1]] = 10;
        ...
        let mut batch_a = TensorImpl::<i32>::zeros(vec![2, 3, 4]);
        let mut batch_b = TensorImpl::<i32>::zeros(vec![2, 4, 3]);
    ...
    #[test]
    fn test_softmax() {
        let mut tensor_2d = TensorImpl::<f32>::zeros(vec![2, 3]);
        tensor_2d[&[0, 0]] = 1.0;
        tensor_2d[&[0, 1]] = 2.0;
        tensor_2d[&[0, 2]] = 3.0;
        tensor_2d[&[1, 0]] = 4.0;
        ...
        let mut tensor_3d = TensorImpl::<f32>::zeros(vec![2, 2, 4]);
        let mut value = 1.0;
        for i in 0..2 {
            for j in 0..2 {
                for k in 0..4 {
    ...
    fn test_one_hot_encode_basic() {
        let indices = TensorImpl::new(vec![0i32, 2, 1], vec![3]);
        let one_hot = TensorImpl::one_hot(&indices, 3);
    ...
    fn test_one_hot_encode_2d() {
        let indices = TensorImpl::new(vec![0i32, 1, 2, 1], vec![2, 2]);
        let one_hot = TensorImpl::one_hot(&indices, 3);
    ...
    fn test_one_hot_invalid_num_classes() {
        let indices = TensorImpl::new(vec![0i32], vec![1]);
        TensorImpl::one_hot(&indices, 0);
    }

    #[test]
    #[should_panic(expected = "Index 3 out of bounds")]
    fn test_one_hot_out_of_bounds() {
        let indices = TensorImpl::new(vec![3i32], vec![1]);
        TensorImpl::one_hot(&indices, 3);
    }
}
